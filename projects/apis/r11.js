...
async function main_r12(upsNo) {
  // query table: ups_info
  var form = {
    select: 'sta, due_inv_dt, inv_ratio, due_dt, out_dt, out_mny, disc_mny, b_upcd',
    no: 'eq.' + upsNo,
  };
  ...
  var form = {
    select: 'no, ups_info_no, sta, cont_term, ups_no, cont_attr1',
  ...
  invIncomeReturn.forEach(function (arr) {
    ...
    if (!erpInvInfoNos.includes(erpInvInfoNo)) {  // check if erpNo matches
