...
async function main(memberNo) {
  ...
  invIncomeReturn.forEach(function (arr) {
    ...
    let item = {
      ...
      repaid: returnOrigin + returnInterest + arr.income_tax + arr.local_tax,  // 상환완료금액
  ...
  for (const upsInvInfoNo of Object.keys(upsInvInfoNo2Items)) {
    ...
    if (arr.length === 1) {  // 완전상환 or full 미상환
      itemsFiltered.push(arr[0]);
    } else {  // 부분 상환 or multi 완전상환
      let filteredItem;
      let returnCount = 0;
      ...
      if (!filteredItem) {  // multi 완전상환 일경우
        filteredItem = Object.assign({}, arr[0]);
      }
