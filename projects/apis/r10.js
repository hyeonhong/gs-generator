...
async function main({ memberNo, upsNos }) {
  // memberNo = 57;
  // upsNos = [3562, 1631];
  ...
  let promises = [];
  for (const upsNo of upsNos) {
    promises.push(main_r8({ memberNo, upsNo }));
  }
  let items = await Promise.all(promises);

  items = items.filter(function(obj) {
    return Object.keys(obj).length > 0; // if object is NOT empty:
  });
...
function orderItems(pageNumber, itemPerPage, pageOrder, items) {
  ...
  if (itemPerPage) {
    ...
    if (pageNumber > numberOfPages) {
      // make sure: page number <= number of pages
...
async function main_r8({ memberNo, upsNo }) {
  // let isValidMember = await db.validateMember(memberNo);
  // if (!isValidMember) {
  //   return {
  //     error: {
  ...
  var form = {
    select:
      'no, ups_info_no, sta, cont_term, cont_attr1, total_loan_mny, out_dt, disc_mny, return_dt, ups_no, b_upcd',
    ups_info_no: 'eq.' + upsNo
  };
  let erpContractInfo = await db.fetch('erp_contract_info', form);
  // validity check for ups_info_no
  if (!erpContractInfo.length) {
    // empty result
    // let result = {
    //   error: {
    //     code: 404,
    //     message: 'ups_info_no not found'
    //   }
    // };
    return {};
  }
  ...
  let productNo = erpContractInfo[0].ups_no;
  ...
  let item = {
    ...
    expected_fee: null, // 예정 수수료
    expected_tax: null, // 예정 세금
    expected_net_profit: null // 예정 실수익
  ...
  if (!bankruptcyFlag) {
    ...
    item.expected_fee = fee; // 예정 수수료
    item.expected_tax = tax; // 예정 세금
    item.expected_net_profit = returnInterest; // 예정 실수익
  }
