...
async function main(memberNo) {
  ...
  let upsInvInfo = await db.fetchRaw('ups_inv_info', form);
  ...
  for (const i of upsInvInfo.keys()) {
    ...
    if (upsNo2Row[upsNo]) {  // 대출 취소/거절
      ...
      let item = {
        ...
        repay_done_date: '',  // 상환완료일
        status: 'rejected',  // 상품상태
        invested: investmentAmt,  // 투자금액
        repaid: investmentAmt,  //상환완료금액
        expected: 0,  // 상환예정금액
        fee: 0,  // 수수료
        tax: 0,  // 세금
        net_profit: 0  // 세후이자
